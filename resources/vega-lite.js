(function (global, factory) {
  ...
  function logger (_, method, handler = log$1) {
    ...
    return {
      ...
      error() {
        if (level >= Error$1) handler('error', 'ERROR', arguments);
        return this;
      },
      warn() {
        if (level >= Warn) handler('warn', 'WARN', arguments);
        return this;
      },
      info() {
        if (level >= Info) handler('log', 'INFO', arguments);
        return this;
      },
      debug() {
        if (level >= Debug) handler('log', 'DEBUG', arguments);
        return this;
      }
    };
  }
  ...
  const duplicate = structuredClone$1;
  ...
  const clonedProto = Object.getPrototypeOf(structuredClone$1({}));
  /**
  ...
  function hasProperty(obj, key) {
      return isObject(obj) && has(obj, key) && obj[key] !== undefined;
  }
  ...
  function selectionAsScaleDomainWithoutField(field) {
      return ('A "field" or "encoding" must be specified when using a selection as a scale domain. ' +
          `Using "field": ${vega.stringValue(field)}.`);
  }
  function selectionAsScaleDomainWrongEncodings(encodings, encoding, extent, field) {
      return (`${!encodings.length ? 'No ' : 'Multiple '}matching ${vega.stringValue(encoding)} encoding found for selection ${vega.stringValue(extent.param)}. ` +
          `Using "field": ${vega.stringValue(field)}.`);
  }
  // CONCAT / REPEAT
  ...
  const clear = {
      ...
      parse: (model, selCmpt) => {
          if (selCmpt.clear) {
              selCmpt.clear = isString(selCmpt.clear) ? vega.parseSelector(selCmpt.clear, 'view') : selCmpt.clear;
          }
      },
      topLevelSignals: (model, selCmpt, signals) => {
  ...
  const legendBindings = {
      ...
      parse: (model, selCmpt, selDef) => {
          ...
          const stream = isString(evt) ? vega.parseSelector(evt, 'view') : array(evt);
  ...
  function disableDirectManipulation(selCmpt, selDef) {
      ...
      if (vega.isString(selDef.select) || !selDef.select.clear)
          delete selCmpt.clear;
      if (vega.isString(selDef.select) || !selDef.select.toggle)
  ...
  function parseUnitSelection(model, selDefs) {
      ...
      for (const def of selDefs) {
          ...
          const selCmpt = (selCmpts[name] = {
              ...
              init: def.value,
              bind: def.bind,
              events: isString(defaults.on) ? vega.parseSelector(defaults.on, 'scope') : array(duplicate(defaults.on)),
          });
          if (isTimerSelection(selCmpt)) {
              nTimerSelections++;
  ...
  */
      ...
      alignStackOrderWithColorDomain() {
          ...
          if (!order && Array.isArray(domain) && typeof field === 'string' && colorEncodingType === 'nominal') {
              ...
              if (offsetEncoding && !offsetEncoding.sort) {
                  ...
                  const orderExpression = `indexof(${$(domain)}, datum['${field}'])`;
  ...
  exports.unescapeSingleQuoteAndPathDot = unescapeSingleQuoteAndPathDot;
